...
from mapleiqa.utils.dataset import *
from mapleiqa.utils.metrics import srocc, plcc
import configparser
...
parser.add_argument("-c", "--config", type=str, help="Set directory of config file. Usually are .ini file.")
parser.add_argument("-v", "--validate", type=str, choices=["best", "latest"], help="Will validate given checkpoint file instead of training.")
    ...
    for key, each_dataloader in dataloader.items():
        if '.train' in key:
            ...
            for img, y in pbar:
                ...
                pbar.set_description(f"Current loader: {key}, Current loss: {(loss.item()):>3f}")
...
def preprocess(koniq10k_df, tid2008_df, tid2013_df, spaq_df, live_df, kadid10k_df):
    ...
    koniq10k_mos = torch.Tensor(koniq10k_df['MOS'].to_numpy())
    ...
    spaq_mos = torch.Tensor(spaq_df['MOS'].to_numpy())

    live_img = LIVE_IMG_DIR + '/' + (live_df['name'].apply(str).str.replace("'", "")).str.strip('[]').to_numpy()
    live_mos = torch.Tensor(live_df['mos'].to_numpy())

    kadid10k_img = KADID10K_IMG_DIR + '/' + kadid10k_df['dist_img'].to_numpy()
    kadid10k_mos = torch.Tensor(kadid10k_df['dmos'].to_numpy())
...
def run():
    ...
    model = build_mapleiqa(LABEL_SET, config['MODEL_CONFIG']['MODEL'], args.config)
    model.to(DEVICE)
    ...
    if args.validate == "best":
        epoch, test_loss = checkpoint(model,
                                    optimizer,
                                    scheduler,
                                    BEST_CHECKPOINT_PATH)
        print(f"Validating from {epoch} epoch, avg val loss: {test_loss:>3f}.")
        validate(dataloaders, model, loss_fn)
    
    elif args.validate == "latest":
        epoch, test_loss = checkpoint(model,
                                    optimizer,
                                    scheduler,
                                    LATEST_CHECKPOINT_PATH)
